...
import { useAddNewEventTypeMutation } from '@/services/api/othersApiSlice';
...
const FormAddEventType = ({ setOpenColapse }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const [addNewEventType, { isLoading, error: errServer }] =
  ...
  const handleOnSubmit = async () => {
    try {
      ...
      const response = await addNewEventType(newData).unwrap();
    ...
    <form onSubmit={handleSubmit(handleOnSubmit)}>
      <div className="grid grid-flow-row-dense grid-cols-1 gap-4 sm:grid-cols-2 gap-y-2">
        <Input
          ...
          errCodeServer="x06040" // already exist
...
export default FormAddEventType;
