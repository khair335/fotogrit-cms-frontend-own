...
import { useAddNewAgeGroupMutation } from '@/services/api/othersApiSlice';

const validationSchema = yup
  .object({
    name: yup.string().required('Age group name is is a required field'),
  })
  .required();

const FormAddAgeGroup = ({ setOpenColapse, optionsGender }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const [addNewAgeGroup, { isLoading, error: errServer }] =
  ...
  const handleOnSubmit = async () => {
    try {
      ...
      const response = await addNewAgeGroup(newData).unwrap();
    ...
    <form onSubmit={handleSubmit(handleOnSubmit)}>
      <div className="grid grid-flow-row-dense grid-cols-1 gap-4 sm:grid-cols-3 gap-y-2">
        <Input
          ...
          errCodeServer="x07022" // already exist
...
export default FormAddAgeGroup;
