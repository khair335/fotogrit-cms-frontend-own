...
import { useUpdateEventPoolMutation } from '@/services/api/othersApiSlice';
...
const FormDetailPool = (props) => {
  ...
  const [updateEventPool, { isLoading, error: errServer }] =
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await updateEventPool(updateData).unwrap();
    ...
    <div>
      <div className="grid grid-cols-1 gap-2 mb-6 text-sm sm:grid-cols-2">
        <div className="flex flex-col gap-2">
          <h5 className="font-bold">Pool Name</h5>
