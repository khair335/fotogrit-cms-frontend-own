...
import { useAddNewEventPoolMutation } from '@/services/api/othersApiSlice';
...
const FormAddPool = ({ setOpenColapse }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const [addNewEventPool, { isLoading, error: errServer }] =
  ...
  const handleOnSubmit = async () => {
    try {
      ...
      const response = await addNewEventPool(newData).unwrap();
