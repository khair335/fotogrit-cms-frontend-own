...
const TableRequestNewService = (props) => {
  const {
    data,
    isLoading,
    isSuccess,
  ...
  const uniqueKey = (row, index) => `rns${row.id}_${index}`;
      ...
      {isError ? (
        ...
        <div className="border-t border-gray-200 ">
          <DataTable
            ...
            keyField={uniqueKey}
            persistTableHead
            noDataComponent={<NoDataMessage title="RNS" />}
          />
        </div>
      ) : null}
    </div>
...
export default TableRequestNewService;
